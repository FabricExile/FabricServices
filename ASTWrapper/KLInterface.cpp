...
KLInterface::KLInterface(const KLFile* klFile, const KLNameSpace * nameSpace, JSONData data)
: KLType(klFile, nameSpace, data)
{
  ...
  if(!m_isForwardDecl)
  {
    ...
    {
      KLMethod * m = new KLMethod(klFile, nameSpace, members->getArrayElement(i), getName());
      pushMethod(m);
    }
  }
}
